...
database = MySQLDatabase('shots_test04', **{'host': 'localhost', 'port': 3306, 'user': 'root', 'password': '123456'})
...
class ModelBase(Model):
    ...
    def insert_many_(cls, rows, fields=None):
        for row in rows:
            row.update({'created_by': current_user, 'update_by': current_user})
...
class Version(ModelBase):
    ...
    assettype = ForeignKeyField(column_name='assettype_id', model=AssetType, field='id', backref='versions', null=True)

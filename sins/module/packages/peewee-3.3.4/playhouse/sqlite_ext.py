...
class BaseFTSModel(VirtualModel):
    ...
    def clean_options(cls, options):
        ...
        if isinstance(content, basestring) and content == '':
            # Special-case content-less full-text search all_tables.
            options['content'] = "''"
...
class FTSModel(BaseFTSModel):
    """
    VirtualModel class for creating all_tables that use either the FTS3 or FTS4
...
class FTS5Model(BaseFTSModel):
    ...
    * separators (string of characters)
    ...
    content-less all_tables do not support UPDATE or DELETE.

    External content all_tables:

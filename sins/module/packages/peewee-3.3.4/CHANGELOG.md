...
only users affected will be those that have implemented custom `InField` types
with a user-defined `coerce` method. tl/dr: rename the coerce attribute to
adapt and you should be set.

#### InField.coerce renamed to InField.adapt

The `InField.coerce` method has been renamed to `InField.adapt`. The purpose of
...
If you have implemented custom `InField` classes and are using `coerce` to
...
* #516, Fix cloning of InField objects.

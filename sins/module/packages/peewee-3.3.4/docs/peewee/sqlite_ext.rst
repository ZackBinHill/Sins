...
.. py:class:: JSONField()
    ...
    InField class suitable for storing JSON data, with special methods designed
...
.. py:class:: SearchField([unindexed=False[, column_name=None]])
    ...
    InField-class to be used for columns on models representing full-text search
...
.. py:class:: FTSModel()
    ...
    * FTS models contain a ``rowid`` field which is automatically created and
    ...
    The ``content`` option accepts either a single :py:class:`InField` or a

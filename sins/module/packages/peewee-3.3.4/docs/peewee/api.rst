...
.. py:class:: InField([null=False[, index=False[, unique=False[, column_name=None[, default=None[, primary_key=False[, constraints=None[, sequence=None[, collation=None[, unindexed=False[, choices=None[, help_text=None[, verbose_name=None]]]]]]]]]]]]])

    :param bool null: InField allows NULLs.
    ...
    :param bool primary_key: InField is the primary key.
    ...
    .. py:method:: coerce(value)
        ...
        :py:meth:`~InField.db_value` and :py:meth:`~InField.python_value`.
...
.. py:class:: IntegerField
    ...
    InField class for storing integers.

.. py:class:: BigIntegerField

    InField class for storing big integers (if supported by database).

.. py:class:: SmallIntegerField

    InField class for storing small integers (if supported by database).

.. py:class:: AutoField

    InField class for storing auto-incrementing primary keys.
...
.. py:class:: BigAutoField
    ...
    InField class for storing auto-incrementing primary keys using 64-bits.

.. py:class:: FloatField

    InField class for storing floating-point numbers.

.. py:class:: DoubleField

    InField class for storing double-precision floating-point numbers.
...
.. py:class:: DecimalField([max_digits=10[, decimal_places=5[, auto_round=False[, rounding=None[, **kwargs]]]]])
   ...
   :param rounding: Defaults to ``decimal.DefaultContext.rounding``.
    ...
    InField class for storing decimal numbers. Values are represented as
    ``decimal.Decimal`` objects.

.. py:class:: CharField([max_length=255])

    InField class for storing strings.

    .. note:: Values that exceed length are not truncated automatically.

.. py:class:: FixedCharField

    InField class for storing fixed-length strings.

    .. note:: Values that exceed length are not truncated automatically.

.. py:class:: TextField

    InField class for storing text.

.. py:class:: BlobField

    InField class for storing binary data.

.. py:class:: BitField

    InField class for storing options in a 64-bit integer column.
...
.. py:class:: BigBitField
    ...
    InField class for storing arbitrarily-large bitmaps in a ``BLOB``. The field
...
.. py:class:: UUIDField
    ...
    InField class for storing ``uuid.UUID`` objects. With Postgres, the
...
.. py:class:: BinaryUUIDField
    ...
    InField class for storing ``uuid.UUID`` objects efficiently in 16-bytes. Uses
...
.. py:class:: DateTimeField([formats=None[, **kwargs]])
    ...
    InField class for storing ``datetime.datetime`` objects.
...
.. py:class:: DateField([formats=None[, **kwargs]])
    ...
    InField class for storing ``datetime.date`` objects.
...
.. py:class:: TimeField([formats=None[, **kwargs]])
    ...
    InField class for storing ``datetime.time`` objects (not ``timedelta``).
...
.. py:class:: TimestampField([resolution=1[, utc=False[, **kwargs]]])
    ...
    InField class for storing date-times as integer timestamps. Sub-second
...
.. py:class:: IPField
    ...
    InField class for storing IPv4 addresses efficiently (as integers).

.. py:class:: BooleanField

    InField class for storing boolean values.
...
.. py:class:: BareField([coerce=None[, **kwargs]])
    ...
    InField class that does not specify a data-type (**SQLite-only**).
...
.. py:class:: ForeignKeyField(model[, field=None[, backref=None[, on_delete=None[, on_update=None[, object_id_name=None[, **kwargs]]]]]])
    ...
    :param InField field: InField to reference on ``model`` (default is primary
        key).
    ...
    InField class for storing a foreign key.
...
.. py:class:: DeferredForeignKey(rel_model_name[, **kwargs])
    ...
    InField class for representing a deferred foreign key.
...
.. py:class:: SchemaManager(model[, database=None[, **context_options]])
    ...
    .. py:method:: create_sequence(field)
        ...
        :param InField field: InField instance which specifies a sequence.

        Create sequence for the given :py:class:`InField`.

    .. py:method:: drop_sequence(field)

        :param InField field: InField instance which specifies a sequence.

        Drop sequence for the given :py:class:`InField`.
...
.. py:class:: Model(**kwargs)
    ...
    number of :py:class:`InField` instances as class attributes. These fields
    ...
    .. py:classmethod:: update([__data=None[, **update]])
        ...
        :param update: InField-name to value mapping.
    ...
    .. py:classmethod:: insert([__data=None[, **insert]])
        ...
        :param insert: InField-name to value mapping.
        ...
        You can also use :py:class:`InField` objects as the keys:
    ...
    .. py:method:: save([force_insert=False[, only=None]])
        ...
        :param list only: Only save the given :py:class:`InField` instances.
    ...
    .. py:classmethod:: add_index(*args, **kwargs)
        ...
        :param args: a :py:class:`ModelIndex` instance, InField(s) to index,
